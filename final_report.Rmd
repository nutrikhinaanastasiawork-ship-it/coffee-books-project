...
```{r top-products}
metrics_A$top_products %>%
  mutate(`Выручка (руб.)` = scales::comma(revenue)) %>%
  select(Товар = product, Категория = category, `Выручка (руб.)`) %>%
  kable(caption = "Топ-5 товаров по выручке")
```
...
abc_table <- metrics_A$abc %>%
  ...
  summarise(
    Товаров = n(),
    `Выручка (руб.)` = scales::comma(sum(revenue)),
    `Доля (%)` = scales::percent(sum(revenue) / total_revenue, accuracy = 0.1)
  ) %>%
  rename(`Класс ABC` = class)
...
**Вывод:** Категория A приносит 70% выручки, категория B — 20%, категория C — 10%.
...
```{r weekday-revenue}
metrics_B$sales_by_weekday %>%
  mutate(
    `Выручка (руб.)` = scales::comma(revenue),
    `Средний чек (руб.)` = scales::comma(avg_check)
  ) %>%
  select(День = weekday, `Выручка (руб.)`, Транзакций = transactions, `Средний чек (руб.)`) %>%
  kable(caption = "Выручка по дням недели")
```
...
metrics_B$weekday_vs_weekend %>%
  mutate(
    `Выручка (руб.)` = scales::comma(revenue),
    `Средний чек (руб.)` = scales::comma(avg_check),
    Тип = ifelse(day_type == "weekday", "Будни", "Выходные")
  ) %>%
  select(Тип, `Выручка (руб.)`, Транзакций = transactions, `Средний чек (руб.)`) %>%
  kable(caption = "Сравнение будней и выходных")
```
